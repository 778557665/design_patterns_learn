组合模式：
    透明组合模式、安全组合模式
        ...
        安全组合模式即在抽象构件Component(AbstractFile)中没有声明任何用于管理成员对象的方法，而是在Composite(Folders)类中声明并实现这些方法。
        这种做法是安全的，因为根本不向叶子对象(Image、Text、Video)提供这些管理成员对象的方法，对于叶子对象，客户端不可能调用到这些方法
